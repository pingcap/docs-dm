...
DDL is a type of SQL statements which is bound to be used in database applications. For MySQL 5.6 and later versions, MySQL supports online-ddl, but there are limitations for usage. For example, to acquire the MDL lock, some DDLs still need to be copied. In production scenario, the table lock during DDL execution can block the reads or writes to the database to a certain extent.
