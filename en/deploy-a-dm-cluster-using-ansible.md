...
| :------------- | :------- |
| source_id | DM-worker binds to a unique database instance or a migration group with the primary-secondary architecture. When the primary and secondary switch, you only need to update `mysql_host` or `mysql_port` and do not need to update the `source_id`. |
