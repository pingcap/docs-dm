...
DM can be deployed and run on a 64-bit general-purpose hardware server platform (Intel x86-64 architecture). For servers used in the development, testing, and production environments, this section illustrates recommended hardware configurations (these do not include the resources used by the operating system).
...
> - If you need to verify the function only, you can deploy a DM-master stand-alone. The number of DM-worker deployed depends on the number of upstream MySQL instances to be replicated.
> - DM-worker stores fulls data in the `dump` and `load` phases. Therefore, the disk space for DM-worker needs to be greater than the total amount of data to be migrated. If the relay log is enabled for the replication task, DM-worker needs additional disk space to store upstream binlog data.
...
> - The production environment requires higher server configurations (than those mentioned in the table above).
