...
| :------ | :--------- | :------------- |
| `source-id`  | Uniquely identifies a MySQL or MariaDB instance, or a migration group with the primary-secondary structure. The maximum length of `source-id` is 32. | `source_id` of `inventory.ini`;<br/> `source-id` of `dm-master.toml`;<br/> `source-id` of `task.yaml` |
