...
## Why does TiUP fail to deploy some certain versions of DM (for example, v2.0.0-hotfix)？

You can use the `tiup list dm-master` command to view the DM versions that TiUP supports to deploy. TiUP cannot manage versions which are not shown by this command.

## How to handle the error `parse mydumper metadata error: EOF` that occurs when DM is synchronizing data？

You need to check the error message and log files to further analyze this error. The cause can be that the dump unit does not produce the correct metadata file due to lack of permissions.

## Why does DM report no fatal error when synchronizing sharded schemas and tables, but downstream data is lost?

You need to check the configuration items `block-allow-list` and `table-route`:

- You need to fill `block-allow-list` with the names of upstream database tables. You can add "~" before `do-tables` to use regular expressions to match table names.
...
## Why does the monitor metric `replicate lag` have no data when DM is not replicating from upstream?
