...
DM 2.0 introduces more fields to meta-information tables such as checkpoint. In DM 2.0, if you directly run the `start-task` command and continue the incremental data replication with the task configuration file of the DM 1.0 cluster, then the error `Error 1054: Unknown column 'binlog_gtid' in 'field list'` occurs.
