...
> **Note:**
> 
> Since DM v2.0.2, the configuration item `enable-relay` in the source configuration file is no longer valid. If DM finds that `enable-relay` is set to `true` when [loading the data source configuration](dm-manage-source.md#operate-data-source), it outputs the following message:
> 
> ```
...
See [Upstream Database Configuration File](dm-source-configuration-file.md) for how to set the configuration item `enable-relay`.
