...
For example, to scale in the DM-worker node on `172.16.5.140`, run the following command. You can scale in the DM-master using similar methods:
...
For example, to scale out a DM-worker node in the `prod-cluster` cluster, take the following steps. You can scale in the DM-master using similar methods:
...
> - TiUP only supports importing to v2.0.0-rc.2 or later versions.
...
> - Only one DM-master node is in the cluster after importing. Refer to [Scale out a cluster](#scale-out-a-cluster) to expand the DM-master.
