---
title: Merge and Replicate Data from Sharded Tables in the Pessimistic Mode
summary: Learn how DM merges and replicates data from sharded tables in the pessimistic mode.
---

# Merge and Replicate Data from Sharded Tables in the Pessimistic Mode

This document introduces the sharding support feature provided by Data Migration (DM) in the pessimistic mode (the default mode). This feature allows you to merge and replicate the data of tables with the same table schema in the upstream MySQL or MariaDB instances into one same table in the downstream TiDB.
...
- If you need to change the [table routing](key-features.md#table-routing) rule, you have to wait for the replication of all sharding DDL statements to complete.
...
This section shows how DM replicates DDL statements in the process of merging sharded tables based on the above example in the pessimistic mode.
