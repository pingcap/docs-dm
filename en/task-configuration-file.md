...
mysql-instances:
  ...
  - source-id: "mysql-replica-01"
    ...
    loader-thread: 16              # The number of threads that Loader uses for loading data. When multiple instances are migrating data to TiDB at the same time, slightly reduce the value according to the load condition.
    syncer-thread: 16              # The number of threads that Syncer uses for replicating incremental data. When multiple instances are migrating data to TiDB at the same time, slightly reduce the value according to the load condition.
