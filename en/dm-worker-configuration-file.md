...
| `keepalive-ttl` | The keepalive time (in seconds) of a DM-worker node to the DM-master node if the upstream data source it binds does not enable the relay log. The default value is 60s.|
| `relay-keepalive-ttl` | The keepalive time (in seconds) of a DM-worker node to the DM-master node if the upstream data source it binds enables the relay log. The default value is 1800s. New in v2.0.2.|
