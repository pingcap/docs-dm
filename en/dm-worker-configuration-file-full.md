...
# Each instance (primary and secondary) in the migration group should have a different server ID.
server-id = 101
...
| `source-id` | Uniquely identifies a MySQL or MariaDB instance, or a migration group | |
| `server-id` | Identifies the server ID of DM-worker as a MySQL or MariaDB secondary library. In a migration group, each instance (primary and secondary included) must have a unique server ID. In v1.0.2 and later versions, the `server_id` is automatically generated by DM. | |
