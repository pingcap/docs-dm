...
# Each instance (master and slave) in migration group should have different server id.
server-id = 101
...
| `server-id` | DM-worker 作为上游 MySQL/MariaDB slave 来获取 binlog 的 server id，该值在一个 migration group （包括 master 和 slave）中是唯一的。v1.0.2 及以上版本的 DM 会自动生成，不需要手动配置该项。 |
