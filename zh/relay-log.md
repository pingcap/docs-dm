...
aliases: ['/docs-cn/dev/reference/tools/data-migration/relay-log/','/docs-cn/v3.1/reference/tools/data-migration/relay-log/','/docs-cn/v3.0/reference/tools/data-migration/relay-log/','/docs-cn/v2.1/reference/tools/data-migration/relay-log/','/docs-cn/stable/reference/tools/data-migration/relay-log/']
---
...
DM-worker 在启动后，会自动将上游 binlog 复制到本地配置目录（若使用 DM-Ansible 部署 DM，则迁移目录默认为 `<deploy_dir> / relay_log` ）。DM-worker 在运行过程中，会将上游 binlog 实时迁移到本地文件。DM-worker 的处理单元 Syncer 会实时读取本地 relay log 的 binlog 事件，将这些事件转换为 SQL 语句，再将 SQL 语句迁移到下游数据库。
