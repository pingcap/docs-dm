...
[Black & white table lists](key-features.md#black--white-table-lists) 是指上游数据库实例表的黑白名单过滤规则。其过滤规则类似于 MySQL `replication-rules-db`/`replication-rules-table`，可以用来过滤或只迁移某些数据库或某些表的所有操作。
...
+ 操作限制
    ...
    - DM-worker 重启后不能自动恢复数据迁移任务，需要使用 dmctl 手动执行 `start-task`。详见[创建数据迁移任务](create-task.md)。
