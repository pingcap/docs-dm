...
DM 是 MySQL 到 TiDB 的数据迁移工具，online-ddl/online-ddl-scheme 功能就是对上述两个 online-schema-change 的工具进行特殊的处理，以便更快完成所需的 DDL 迁移。

如果想从源码方面了解 DM online-ddl/online-ddl-scheme，可以参考 [DM 源码阅读系列文章（八）Online Schema Change 迁移支持](https://pingcap.com/blog-cn/dm-source-code-reading-8/#dm-源码阅读系列文章八online-schema-change-迁移支持)

## 配置

online-ddl/online-ddl-scheme 在 task 配置文件里面与 name 同级，例子详见下面配置 Example。完整的配置及意义，可以参考 [DM 完整配置文件示例](task-configuration-file-full.md#完整配置文件示例)：
...
online-ddl: true                # 目前仅支持 gh-ost 、pt
online-ddl-scheme: "gh-ost"     # online-ddl-scheme 将在未来被弃用，建议使用 online-ddl
