...
当前版本的 DM 提供了丰富多样的功能特性，包括 [Table Routing](key-features.md#table-routing)，[Block & Allow Lists](key-features.md#block-allow-table-list)，[Binlog Event Filter](key-features.md#binlog-event-filter) 等。但这些功能特性同时也增加了用户使用 DM 的复杂度，尤其在进行 [DM 任务配置](task-configuration-file.md)的过程中。
