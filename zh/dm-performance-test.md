...
aliases: ['/docs-cn/tidb-data-migration/dev/performance-test/','/zh/tidb-data-migration/dev/performance-test.md/]
---
...
1. 创建上游 MySQL 的 source，将 `source-id` 配置为 `source-1`。详细操作方法参考：[加载数据源配置](dm-manage-source.md#数据源操作)。
...
创建数据迁移任务的详细操作参考[创建数据迁移任务](dm-create-task.md#创建数据迁移任务)。
...
1. 创建上游 MySQL 的 source, source-id 配置为 `source-1`（如果在全量迁移性能测试中已经创建，则不需要再次创建）。详细操作方法参考：[加载数据源配置](dm-manage-source.md#数据源操作)。
