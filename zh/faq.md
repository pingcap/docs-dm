...
## DM v1.0 在任务出错时 sql-skip 无法跳过某些语句

首先需要检查 sql-skip 之后 binlog 位置是否在推进，如果是的话表示 sql-skip 已经生效。重复出错的原因是上游发送了多个不支持的 DDL，可以通过 `sql-skip -s <sql-pattern>` 进行模式匹配。
...
是 TiDB parser 无法解析上游的 DDL，例如 ALTER EVENT，sql-skip 不会按预期生效。可以在任务配置文件中添加 [Binlog 过滤规则](https://docs.pingcap.com/zh/tidb-data-migration/stable/key-features#binlog-event-filter)进行过滤（注意 `schema-pattern: "*"）。

在 DM v2.0 版本中 sql-skip 已经被 handle-error 替代，handle-error 可以跳过该类错误。

## DM 同步时下游长时间出现 REPLACE 语句

请检查是否符合 [safe mode 触发条件](https://docs.pingcap.com/zh/tidb-data-migration/stable/glossary#safe-mode)。如果任务发生错误并自动恢复，或者高可用调度，会满足“启动或恢复任务”的前 5 分钟，因此启用 safe mode。
