...
2. DM 任务开启 relay log 选项，保证 binlog 被清理后 DM 仍有 relay log 读取。
...
- 方法一：使用 `tiup update --self && tiup update dm` 升级 TiUP 到更新版本，随后先 scale-in 再 scale-out grafana 节点，重建 grafana 服务。
- 方法二：
